    ...
    return {
        ...
        createChartData: function (selection, settings, bins) {
            ...
            for (var j = 0; j < selection.length; j++) {
                totalrange.push(selection[j].attributes[settings.areaname2]);
            }
            ...
            var isSelected = "";
            var checkNan = isNaN(kernelwidth);
            ...
            function removeDecimalPlaces(minValue) {

                var toString = minValue.toString();
                var string = toString.substring(toString.indexOf(".") + 1);
                var addZeroo = "0." + string;
                return Number(addZeroo);
            }
            ...
            if (minMaxCheck === "insertValue") {
                color = ["#f703ff"]
                var colorList = d3.quantize(d3.interpolateHcl("#FBE789", "#1B90A7"), Math.round(10));
                var removeDecimal = removeDecimalPlaces(minValue);
                var numberInterpolate = d3.interpolateNumber(1, 10);
                var InterpolateNumber = Math.round(numberInterpolate(removeDecimal)) - 1;
                var newList = [];
                var colorListInterpolation = colorList[InterpolateNumber];
                newList.push(colorListInterpolation);
                color = newList;
            }
            if (kernel.length > 1) {
        ...
        createChart: function (selection, data, settings, state, view, callback) {
            ...
            var chart = AmCharts.makeChart("chartDiv", {
                ...
                "categoryAxis": {
                    ...
                    "tickPosition": "start",
                    "tickLength": 20,
                    "title": "Plocha jednotlivých prvků",
                    "titleBold": false
                },
                "export": {
                    "enabled": true
            ...
            chart.addListener("clickGraphItem", function (event) {
                ...
                settings.layer1.renderer = applyRenderer.createRendererVVbar(min, max, color, settings.areaname2);
